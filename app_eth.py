...
def load_data(interval: str, limit: int) -> pd.DataFrame:
    client = get_client()
    # <-- CHANGE SYMBOL HERE for Ethereum
...
# ─── Display ────────────────────────────────────────────────────────────────
st.title("🔹 Ethereum Hybrid Strategy Dashboard")
...
st.download_button("💾 Download CSV", data=csv, file_name=f"eth_signals_{interval}.csv")
